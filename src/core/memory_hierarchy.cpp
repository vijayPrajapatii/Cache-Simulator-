...
MemoryHierarchy::MemoryHierarchy(const MemoryHierarchyConfig &config)
    : l1(config.l1Config), stridePredictor(config.strideTableSize),
      prefetchDistance(config.l1Config.prefetchDistance),
      prefetchEnabled(config.l1Config.prefetchEnabled),
      l3Inclusive_(config.l3Inclusive), l1Reads(0), l1Writes(0), l1Misses(0),
      l2Accesses(0), l2Misses(0), l3Accesses(0), l3Misses(0),
      usefulPrefetches(0), uselessPrefetches(0) {
  ...
  // Initialize L3 cache if configured
  if (config.l3Config) {
    l3.emplace(*config.l3Config);
  }
...
void MemoryHierarchy::access(uint32_t address, bool isWrite) {
  ...
  std::optional<Cache *> nextLevelPtr =
      l2 ? std::optional<Cache *>(&*l2) : std::nullopt;
  ...
  bool l1Hit = l1.access(
      address, isWrite, nextLevelPtr,
      prefetchEnabled ? std::optional<StridePredictor *>(&stridePredictor)
                      : std::nullopt);
  ...
  if (!l1Hit) {
    ...
    if (l2) {
      l2Accesses++;
      bool l2Hit = l2->access(address, isWrite, std::nullopt);
      ...
      if (!l2Hit) {
        ...
        if (l3) {
          l3Accesses++;
          bool l3Hit = l3->access(address, isWrite, std::nullopt);

          if (!l3Hit) {
            l3Misses++;
            // Memory access would happen here
          }
        }
      }
    } else if (l3) {
      // No L2, but L3 configured (unusual but supported)
      l3Accesses++;
      bool l3Hit = l3->access(address, isWrite, std::nullopt);
      if (!l3Hit) {
        l3Misses++;
...
void MemoryHierarchy::processTrace(const std::string &traceFilePath) {
  ...
  if (!std::filesystem::exists(traceFilePath)) {
    std::cerr << "Error: Trace file " << traceFilePath << " does not exist."
              << std::endl;
    return;
  }
  ...
  std::cout << "Completed processing " << accessCount << " memory accesses."
            << std::endl;
}
...
double MemoryHierarchy::getL1MissRate() const {
  ...
  return totalAccesses > 0 ? static_cast<double>(l1Misses) / totalAccesses
                           : 0.0;
}

double MemoryHierarchy::getPrefetchAccuracy() const {
  const int totalPrefetches = usefulPrefetches + uselessPrefetches;
  return totalPrefetches > 0
             ? static_cast<double>(usefulPrefetches) / totalPrefetches
             : 0.0;
}
...
void MemoryHierarchy::printStats() const {
  ...
  // Print L3 cache statistics if present
  if (l3) {
    std::cout << std::endl << "L3 Cache:" << std::endl;
    l3->printStats();
  }
  ...
  if (l2) {
    ...
    std::cout << "  L2 Miss Rate: " << std::fixed << std::setprecision(2)
              << (getL2MissRate() * 100.0) << "%" << std::endl;
  }

  if (l3) {
    std::cout << "  L3 Misses: " << l3Misses << std::endl;
    std::cout << "  L3 Miss Rate: " << std::fixed << std::setprecision(2)
              << (getL3MissRate() * 100.0) << "%" << std::endl;
  }
...
void MemoryHierarchy::compareWith(const MemoryHierarchy &other) const {
  ...
  if (otherMissRate > 0) {
    missRateImprovement =
        (otherMissRate - thisMissRate) / otherMissRate * 100.0;
  }
  ...
  if (otherMemTraffic > 0) {
    memTrafficReduction =
        static_cast<double>(otherMemTraffic - thisMemTraffic) /
        otherMemTraffic * 100.0;
  }
  ...
  std::cout << "  Configuration 1: " << thisMemTraffic << " misses"
            << std::endl;
  std::cout << "  Configuration 2: " << otherMemTraffic << " misses"
            << std::endl;
...
double MemoryHierarchy::getL3HitRate() const {
  if (!l3) {
    return 0.0;
  }
  return l3->getHitRatio();
}

double MemoryHierarchy::getL3MissRate() const {
  if (!l3) {
    return 0.0;
  }
  return l3->getMissRatio();
}

void MemoryHierarchy::handleL3Eviction([[maybe_unused]] uint32_t address) {
  // For inclusive L3, evicting from L3 requires back-invalidation
  // of the same line in L1 and L2 (if present)
  // This is a placeholder for future implementation
  if (l3Inclusive_) {
    // Future: invalidate address in L1 and L2
  }
}
