...
Cache::Cache(const CacheConfig &config)
    : size(config.size), associativity(config.associativity),
      blockSize(config.blockSize), replacementPolicy(config.replacementPolicy),
      writePolicy(config.writePolicy), streamBuffer(config.prefetchDistance),
      prefetchEnabled(config.prefetchEnabled), hits(0), misses(0), reads(0),
      writes(0), writeBacks(0), writeThroughs(0), missTypeStats{0, 0, 0, 0} {
  ...
  if (numSets <= 0) {
    throw CacheConfigError(
        "Invalid configuration: number of sets must be positive");
  }
  ...
  for (int setIdx = 0; setIdx < numSets; ++setIdx) {
    ...
    replacementPolicies[setIdx] =
        ReplacementPolicyFactory::create(replacementPolicy, associativity);
  }
}
...
bool Cache::access(uint32_t address, bool isWrite,
  ...
  if (auto blockIndex = std::get_if<int>(&result)) {
    ...
    if (prefetchEnabled && streamBuffer.access(address)) {
      ...
      // FUTURE: Add optional verbose logging via Logger class
      if constexpr (false) { // Disable verbose output in production
        std::cout << "Stream buffer hit: 0x" << std::hex << address << std::dec
                  << std::endl;
      }
    ...
    if (victimBlock.valid && victimBlock.requiresWriteback()) {
      uint32_t victimAddress =
          (victimBlock.tag * numSets + setIndex) * blockSize;
      writeBack(victimAddress, nextLevel.value_or(nullptr));
    }

    // Install the new block
    installBlock(address, setIndex, victimIndex, isWrite,
                 nextLevel.value_or(nullptr), false);
    ...
    if (prefetchEnabled && stridePredictor) {
      handlePrefetch(address, isWrite, nextLevel.value_or(nullptr),
                     stridePredictor.value_or(nullptr));
    }
...
void Cache::installBlock(uint32_t address, int setIndex, int blockIndex,
                         bool isWrite, Cache *nextLevel, bool isPrefetch) {
  ...
  if (nextLevel) {
    [[maybe_unused]] bool hitStatus =
        nextLevel->access(address, false, std::nullopt);
  }
...
void Cache::handlePrefetch(uint32_t address, bool isWrite, Cache *nextLevel,
  ...
  if (!isWrite && stridePredictor) {
    ...
    if (stride != 0) {
      ...
      // FUTURE: Add optional verbose logging via Logger class
      if constexpr (false) { // Disable verbose output in production
        std::cout << "Prefetching address (stride-based): 0x" << std::hex
                  << prefetchAddress << std::dec << std::endl;
      }
      ...
      if (!std::holds_alternative<int>(result)) {
        ...
        installBlock(prefetchAddress, setIndex, victimIndex, false, nextLevel,
                     true);
...
void Cache::printStats() const {
  ...
  std::cout << "  Replacement Policy: "
            << replacementPolicyToString(replacementPolicy) << std::endl;
  std::cout << "  Write Policy: "
            << (writePolicy == WritePolicy::WriteBack ? "Write-back"
                                                      : "Write-through")
            << std::endl;
  std::cout << "  Prefetching: " << (prefetchEnabled ? "Enabled" : "Disabled")
            << std::endl;
  ...
  for (int i = 0; i < 4; ++i) {
    std::cout << "  " << CacheMissTypeNames[i]
              << " Misses: " << missTypeStats[i] << " (" << std::fixed
              << std::setprecision(2)
...
std::string Cache::exportCacheState() const {
  ...
  oss << "  Replacement Policy: "
      << replacementPolicyToString(replacementPolicy) << "\n";
  oss << "  Write Policy: "
      << (writePolicy == WritePolicy::WriteBack ? "Write-back"
                                                : "Write-through")
      << "\n\n";
...
bool Cache::isBlockValid(uint32_t setIndex, uint32_t wayIndex) const {
  if (setIndex >= static_cast<uint32_t>(numSets) ||
      wayIndex >= static_cast<uint32_t>(associativity)) {
    return false;
  }
...
uint32_t Cache::getBlockAccessCount(uint32_t setIndex,
                                    uint32_t wayIndex) const {
  if (setIndex >= static_cast<uint32_t>(numSets) ||
      wayIndex >= static_cast<uint32_t>(associativity)) {
    return 0;
  }
