...
public:
  ...
  struct Config {
    ...
    // v1.4.0: Protocol and interconnect selection
    std::string coherenceProtocol = "MESI"; ///< MSI, MESI, MOESI
    std::string interconnectType = "Bus"; ///< Bus, Crossbar, Mesh, Ring, Torus
  };
   ...
   */
  uint32_t processAtomicAccess(uint32_t processorId, uint32_t address,
                               bool isWrite);
