/**
 * @file cache.h
 * @brief Cache implementation with configurable parameters and advanced
 * features
  ...
  void validate() const {
    ...
    if (size % (associativity * blockSize) != 0) {
      throw CacheConfigError(
          "Cache size must be divisible by (associativity * blockSize)");
    }
...
public:
  ...
  [[nodiscard]] bool
  access(uint32_t address, bool isWrite,
  ...
  [[nodiscard]] uint32_t getBlockTag(uint32_t setIndex,
                                     uint32_t wayIndex) const;
  [[nodiscard]] uint32_t getBlockAccessCount(uint32_t setIndex,
                                             uint32_t wayIndex) const;
  [[nodiscard]] uint64_t getBlockLastAccess(uint32_t setIndex,
                                            uint32_t wayIndex) const;
  [[nodiscard]] bool isBlockPrefetched(uint32_t setIndex,
                                       uint32_t wayIndex) const;
...
private:
  ...
  std::vector<std::unique_ptr<ReplacementPolicyBase>>
      replacementPolicies; // v1.1.0
...
private:
  ...
  int findVictim(const CacheSet &set,
                 int setIndex) const; // v1.1.0 - added setIndex
  void installBlock(uint32_t address, int setIndex, int blockIndex,
                    bool isWrite, Cache *nextLevel, bool isPrefetch = false);

  // C++17 variant to represent either a hit or miss
  using AccessResult =
      std::variant<int, CacheMissType>; // hit: block index, miss: type

  // Check if block is present in cache
  AccessResult findBlock(uint32_t address) const;

  // Prefetching logic
  void handlePrefetch(uint32_t address, bool isWrite, Cache *nextLevel,
