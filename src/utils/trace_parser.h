...
struct MemoryAccess {
    ...
    std::optional<uint32_t> coreId;     // processor core ID (for multiprocessor traces)

    // Default constructor
    MemoryAccess() = default;

    // Constructor with parameters (single-core)
    MemoryAccess(bool isWrite, uint32_t address)
        : isWrite(isWrite), address(address), coreId(std::nullopt) {}

    // Constructor with core ID (multiprocessor)
    MemoryAccess(bool isWrite, uint32_t address, uint32_t coreId)
        : isWrite(isWrite), address(address), coreId(coreId) {}
};
