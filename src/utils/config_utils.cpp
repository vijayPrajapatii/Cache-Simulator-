...
// library Current implementation handles all standard configuration options
// correctly
...
std::optional<ConfigManager::SimulatorConfig>
ConfigManager::loadConfig(const std::filesystem::path &configPath) {
  if (!std::filesystem::exists(configPath)) {
    std::cerr << "Error: Configuration file " << configPath
  ...
  try {
    ...
    case ConfigFormat::CommandLine:
      std::cerr << "Error: Cannot load command line format from file."
                << std::endl;
      return std::nullopt;
    }
...
bool ConfigManager::saveConfig(const std::filesystem::path &configPath,
...
std::optional<ConfigManager::SimulatorConfig>
ConfigManager::createFromCommandLine(int argc, char *argv[]) {
  if (argc < 8) {
  ...
  try {
    ...
    if (l2Size > 0 && !isValidAssociativity(l2Assoc)) {
      std::cerr
          << "Invalid L2 associativity: must be positive when L2 is enabled"
          << std::endl;
      return std::nullopt;
    }

    if (prefetchEnabled && prefetchDistance <= 0) {
      std::cerr << "Invalid prefetch distance: must be positive when "
    ...
    // Create L1 config
    CacheConfig l1Config{l1Size, l1Assoc, blockSize, prefetchEnabled,
                         prefetchDistance};

    // Create L2 config if needed
    std::optional<CacheConfig> l2Config;
    if (l2Size > 0) {
      l2Config = CacheConfig{l2Size, l2Assoc, blockSize, prefetchEnabled,
                             prefetchDistance};
    }
...
ConfigManager::SimulatorConfig ConfigManager::getDefaultConfig() {
  ...
  // Create L2 config
  CacheConfig l2Config{l2Size, l2Assoc, blockSize, prefetchEnabled,
                       prefetchDistance};
...
ConfigManager::compareConfigs(const SimulatorConfig &config1,
                              const SimulatorConfig &config2) {
  std::vector<std::string> differences;

  // Compare L1 cache parameters
  if (config1.hierarchyConfig.l1Config.size !=
      config2.hierarchyConfig.l1Config.size) {
    differences.push_back(
        "L1 size: " + std::to_string(config1.hierarchyConfig.l1Config.size) +
        " vs " + std::to_string(config2.hierarchyConfig.l1Config.size));
  }

  if (config1.hierarchyConfig.l1Config.associativity !=
      config2.hierarchyConfig.l1Config.associativity) {
    differences.push_back(
        "L1 associativity: " +
        std::to_string(config1.hierarchyConfig.l1Config.associativity) +
        " vs " +
        std::to_string(config2.hierarchyConfig.l1Config.associativity));
  }

  if (config1.hierarchyConfig.l1Config.blockSize !=
      config2.hierarchyConfig.l1Config.blockSize) {
    differences.push_back(
        "Block size: " +
        std::to_string(config1.hierarchyConfig.l1Config.blockSize) + " vs " +
        std::to_string(config2.hierarchyConfig.l1Config.blockSize));
  }

  if (config1.hierarchyConfig.l1Config.prefetchEnabled !=
      config2.hierarchyConfig.l1Config.prefetchEnabled) {
    differences.push_back(
        "Prefetching: " +
        std::string(config1.hierarchyConfig.l1Config.prefetchEnabled
                        ? "enabled"
                        : "disabled") +
        " vs " +
        std::string(config2.hierarchyConfig.l1Config.prefetchEnabled
                        ? "enabled"
                        : "disabled"));
  }

  if (config1.hierarchyConfig.l1Config.prefetchDistance !=
      config2.hierarchyConfig.l1Config.prefetchDistance) {
    differences.push_back(
        "Prefetch distance: " +
        std::to_string(config1.hierarchyConfig.l1Config.prefetchDistance) +
        " vs " +
        std::to_string(config2.hierarchyConfig.l1Config.prefetchDistance));
  }
  ...
  if (config1HasL2 != config2HasL2) {
    differences.push_back(
        "L2 cache: " + std::string(config1HasL2 ? "present" : "absent") +
        " vs " + std::string(config2HasL2 ? "present" : "absent"));
  } else if (config1HasL2 && config2HasL2) {
    // Both have L2, compare parameters
    if (config1.hierarchyConfig.l2Config->size !=
        config2.hierarchyConfig.l2Config->size) {
      differences.push_back(
          "L2 size: " + std::to_string(config1.hierarchyConfig.l2Config->size) +
          " vs " + std::to_string(config2.hierarchyConfig.l2Config->size));
    }

    if (config1.hierarchyConfig.l2Config->associativity !=
        config2.hierarchyConfig.l2Config->associativity) {
      differences.push_back(
          "L2 associativity: " +
          std::to_string(config1.hierarchyConfig.l2Config->associativity) +
          " vs " +
          std::to_string(config2.hierarchyConfig.l2Config->associativity));
    }
  }

  // Compare prefetching strategies
  if (config1.hierarchyConfig.useStridePrediction !=
      config2.hierarchyConfig.useStridePrediction) {
    differences.push_back(
        "Stride prediction: " +
        std::string(config1.hierarchyConfig.useStridePrediction ? "enabled"
                                                                : "disabled") +
        " vs " +
        std::string(config2.hierarchyConfig.useStridePrediction ? "enabled"
                                                                : "disabled"));
  }

  if (config1.hierarchyConfig.useAdaptivePrefetching !=
      config2.hierarchyConfig.useAdaptivePrefetching) {
    differences.push_back(
        "Adaptive prefetching: " +
        std::string(config1.hierarchyConfig.useAdaptivePrefetching
                        ? "enabled"
                        : "disabled") +
        " vs " +
        std::string(config2.hierarchyConfig.useAdaptivePrefetching
                        ? "enabled"
                        : "disabled"));
  }
...
std::vector<ConfigManager::SimulatorConfig>
ConfigManager::generateParameterSweep(
    const SimulatorConfig &baseConfig, const std::string &paramName,
      ...
      if (sizeValue > 0) {
        if (newConfig.hierarchyConfig.l2Config) {
          ...
          CacheConfig l2Config{
              sizeValue, 8, newConfig.hierarchyConfig.l1Config.blockSize,
      ...
      std::visit(
          [&](const auto &val) {
            ...
            if constexpr (std::is_same_v<T, int>) {
              ...
              newConfig.extraOptions[paramName] = val;
              newConfig.name +=
                  val; // For strings, simply append the string directly
            }
          },
          value);
    }
...
std::optional<ConfigManager::SimulatorConfig>
ConfigManager::parseConfigString(std::string_view configStr) {
  ...
  if (tokens.size() < 6) {
    std::cerr << "Invalid config string format. Expected "
                 "l1size:l1assoc:l2size:l2assoc:blocksize:prefetch[:distance]"
              << std::endl;
    return std::nullopt;
  }
  ...
  try {
    ...
    bool prefetchEnabled =
        (tokens[5] == "1" || tokens[5] == "true" || tokens[5] == "yes");
...
bool ConfigManager::validateConfig(const SimulatorConfig &config,
  ...
  if (config.hierarchyConfig.l1Config.prefetchEnabled &&
      config.hierarchyConfig.l1Config.prefetchDistance <= 0) {
    errorMessage =
        "Prefetch distance must be positive when prefetching is enabled";
    return false;
  }
  ...
  if (config.hierarchyConfig.l2Config) {
    ...
    if (config.hierarchyConfig.l2Config->blockSize !=
        config.hierarchyConfig.l1Config.blockSize) {
...
std::optional<ConfigManager::SimulatorConfig>
ConfigManager::parseJsonConfig(const std::filesystem::path &configPath) {
  ...
  while (std::getline(file, line)) {
    ...
    if (colonPos != std::string::npos) {
      ...
      if (currentSection == "perCoreL1" || currentSection == "l1" ||
        ...
        if (key == "size") {
          ...
          if (value == "LRU") {
            config.hierarchyConfig.l1Config.replacementPolicy =
          ...
          if (value == "WriteBack") {
            config.hierarchyConfig.l1Config.writePolicy =
        ...
        if (key == "size") {
          ...
          if (value == "LRU") {
            config.hierarchyConfig.l2Config->replacementPolicy =
                ReplacementPolicy::LRU;
          } else if (value == "NRU") {
            config.hierarchyConfig.l2Config->replacementPolicy =
                ReplacementPolicy::NRU;
          ...
          if (value == "WriteBack") {
            config.hierarchyConfig.l2Config->writePolicy =
                WritePolicy::WriteBack;
          } else if (value == "WriteThrough") {
            config.hierarchyConfig.l2Config->writePolicy =
                WritePolicy::WriteThrough;
  ...
  if (config.hierarchyConfig.l2Config) {
    if (config.hierarchyConfig.l2Config->blockSize == 0) {
      config.hierarchyConfig.l2Config->blockSize =
          config.hierarchyConfig.l1Config.blockSize;
    }
...
bool ConfigManager::writeJsonConfig(const std::filesystem::path &configPath,
                                    const SimulatorConfig &config) {
  // This is a simplified implementation
  ...
  file << "  \"block_size\": " << config.hierarchyConfig.l1Config.blockSize
       << ",\n";
  file << "  \"prefetch_enabled\": "
       << (config.hierarchyConfig.l1Config.prefetchEnabled ? "true" : "false")
       << ",\n";
  file << "  \"prefetch_distance\": "
       << config.hierarchyConfig.l1Config.prefetchDistance << ",\n";
  ...
  file << "  \"use_stride_prediction\": "
       << (config.hierarchyConfig.useStridePrediction ? "true" : "false")
       << ",\n";
  file << "  \"use_adaptive_prefetching\": "
       << (config.hierarchyConfig.useAdaptivePrefetching ? "true" : "false")
       << ",\n";
  ...
  for (const auto &[key, value] : config.extraOptions) {
    try {
      ...
      std::cerr << "Error casting value for key '" << key << "': " << e.what()
                << std::endl;
      file << "  \"" << key << "\": \"ERROR\",\n";
    }
  }
...
std::optional<ConfigManager::SimulatorConfig>
ConfigManager::parseIniConfig(const std::filesystem::path &configPath) {
  // This is a simplified implementation
    ...
    if (pos != std::string::npos) {
      ...
      if (currentSection == "L1Cache") {
        if (key == "size") {
          ...
          config.hierarchyConfig.l1Config.prefetchEnabled =
              (value == "1" || value == "true" || value == "yes");
        } else if (key == "prefetch_distance") {
          config.hierarchyConfig.l1Config.prefetchDistance = std::stoi(value);
        } else if (key == "replacement_policy" || key == "replacementPolicy") {
          if (value == "LRU") {
            config.hierarchyConfig.l1Config.replacementPolicy =
                ReplacementPolicy::LRU;
          } else if (value == "NRU") {
        ...
        if (key == "size") {
          ...
          config.hierarchyConfig.l2Config->prefetchEnabled =
              (value == "1" || value == "true" || value == "yes");
        } else if (key == "prefetch_distance") {
        ...
        if (key == "use_stride_prediction") {
          config.hierarchyConfig.useStridePrediction =
              (value == "1" || value == "true" || value == "yes");
        } else if (key == "use_adaptive_prefetching") {
          config.hierarchyConfig.useAdaptivePrefetching =
              (value == "1" || value == "true" || value == "yes");
...
bool ConfigManager::writeIniConfig(const std::filesystem::path &configPath,
                                   const SimulatorConfig &config) {
  std::ofstream file(configPath);
  if (!file) {
    return false;
  ...
  if (config.hierarchyConfig.l2Config) {
    ...
    file << "associativity = " << config.hierarchyConfig.l2Config->associativity
         << "\n";
    file << "prefetch_enabled = "
         << (config.hierarchyConfig.l2Config->prefetchEnabled ? "true"
                                                              : "false")
         << "\n";
    file << "prefetch_distance = "
         << config.hierarchyConfig.l2Config->prefetchDistance << "\n\n";
  } else {
    file << "[L2Cache]\n";
    file << "size = 0\n";
  ...
  for (const auto &[key, value] : config.extraOptions) {
    ...
    auto pos = key.find('.');
    std::string section =
        (pos != std::string::npos) ? key.substr(0, pos) : "Extra";
    std::string optionKey =
        (pos != std::string::npos) ? key.substr(pos + 1) : key;
    ...
    try {
      ...
      // Handle potential casting errors
      std::cerr << "Error writing option " << optionKey << ": " << e.what()
                << std::endl;
      file << "ERROR";
    }
    file << "\n";
