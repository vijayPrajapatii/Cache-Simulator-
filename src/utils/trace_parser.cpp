...
TraceParser::TraceParser(const std::filesystem::path &filepath)
    : filepath(filepath), fileValid(false), traceFormat(TraceFormat::Simple),
      totalAccesses(0), readAccesses(0), writeAccesses(0), lineNumber(0) {
...
ParseResult TraceParser::parseLine(std::string_view line) {
  ...
  auto commentPos = lineStr.find('#');
  if (commentPos != std::string::npos) {
    lineStr = lineStr.substr(0, commentPos);
  }

  // Trim trailing whitespace
  while (!lineStr.empty() && std::isspace(lineStr.back())) {
    lineStr.pop_back();
  }
...
ParseResult TraceParser::parseJSONLine(std::string_view line) {
  ...
  // Using escaped string instead of raw string literal to avoid compilation
  // issues
  std::regex jsonRegex("\\{\\s*\"type\"\\s*:\\s*\"([rRwW])\"\\s*,\\s*"
                       "\"address\"\\s*:\\s*\"(0x[0-9a-fA-F]+)\"\\s*\\}");
  std::smatch matches;
  std::string lineStr(line);
...
std::optional<uint32_t> TraceParser::parseAddress(std::string_view addressStr) {
  ...
  uint32_t address = 0;
  auto result = std::from_chars(
      cleanAddr.c_str(), cleanAddr.c_str() + cleanAddr.size(), address, 10);
  if (result.ec == std::errc()) {
    return address;
  }
