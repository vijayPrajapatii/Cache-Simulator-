...
struct TraceStatistics {
  ...
  std::unordered_map<uint32_t, size_t>
      accessFrequency; // Access count per address

  // Pattern detection results
  enum class PatternType { Sequential, Strided, Random, LoopingAccess, Mixed };
...
public:
  ...
  [[nodiscard]] std::vector<TraceStatistics::AddressRange>
  getHotRanges(size_t numRanges = 5, uint32_t rangeSize = 4096);

  // Print analysis results
  void printAnalysis(const TraceStatistics &stats) const;

  // Generate a new trace file with filtered accesses
  bool generateFilteredTrace(
...
private:
  ...
  [[nodiscard]] bool
  isSequentialPattern(const std::vector<MemoryAccess> &accesses,
                      double &confidence) const;
  [[nodiscard]] std::optional<int32_t>
  detectStride(const std::vector<MemoryAccess> &accesses,
               double &confidence) const;
  [[nodiscard]] bool isLoopingPattern(const std::vector<MemoryAccess> &accesses,
                                      size_t &loopSize,
                                      double &confidence) const;
};
...
bool generateSequentialTrace(const std::filesystem::path &outputPath,
                             uint32_t startAddress, size_t count,
                             uint32_t stride = 4, double writeRatio = 0.2);

// Generate a random access pattern trace
bool generateRandomTrace(const std::filesystem::path &outputPath,
                         uint32_t minAddress, uint32_t maxAddress, size_t count,
...
bool generateLocalityTrace(const std::filesystem::path &outputPath,
                           size_t numRegions, uint32_t regionSize,
                           size_t accessesPerRegion, double writeRatio = 0.2);

// Generate a mixed pattern trace
bool generateMixedTrace(
    const std::filesystem::path &outputPath,
    const std::vector<std::pair<std::string, double>> &patternMix,
    size_t totalAccesses, double writeRatio = 0.2);
} // namespace trace_generator
