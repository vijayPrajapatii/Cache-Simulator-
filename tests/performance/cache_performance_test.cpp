...
public:
  static void benchmarkBasicCacheOperations() {
    ...
    for (const auto &config : configs) {
      ...
      std::cout << "  " << config.size / 1024 << "KB " << config.associativity
                << "-way: " << accessesPerSecond << " accesses/sec"
                << std::endl;
    }
  ...
  static void benchmarkReplacementPolicies() {
    ...
    std::vector<ReplacementPolicy> policies = {
        ...
        ReplacementPolicy::Random, ReplacementPolicy::NRU};
  ...
  static void benchmarkMemoryHierarchy() {
    ...
    std::cout << "  Memory hierarchy: " << accessesPerSecond << " accesses/sec"
              << std::endl;
    std::cout << "  L1 hit rate: " << (hierarchy.getL1HitRate() * 100) << "%"
              << std::endl;
  ...
  static void benchmarkTraceProcessing() {
    ...
    // Use scoped block to ensure TraceParser is destroyed before file deletion
